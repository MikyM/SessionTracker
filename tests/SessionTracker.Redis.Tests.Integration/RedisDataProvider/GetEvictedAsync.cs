...
public abstract partial class RedisDataProvider
{
    ...
    public abstract class GetEvictedAsyncShould<TFixture>(TFixture fixture) : IClassFixture<TFixture> where TFixture : RedisFixture
    {
        [Fact]
        public async Task ReturnExistingSession()
        {
            ...
            // Act
            var result = await sut.GetEvictedAsync<TestSession>(session.Key);
